...
from sdc.cryptography.invalid_token_exception import InvalidTokenException
from sdc.cryptography.token_helper import decrypt_jwe, decode_jwt
from sdc.cryptography.secrets import SecretStore, KEY_PURPOSE_AUTHENTICATION
...
class TestTokenHelper(TestCase):  # pylint: disable=too-many-public-methods
    ...
    def test_missing_kid_uses_default(self):
        ...
        jwt = decrypt_jwe(jwe.decode(), self.secret_store, KEY_PURPOSE_AUTHENTICATION,
    ...
    def test_jwt_io(self):
        token = decode_jwt(jwtio_signed, self.secret_store, purpose=KEY_PURPOSE_AUTHENTICATION, check_claims=self.CHECK_CLAIMS)
    ...
    def assert_in_decode_signed_jwt_exception(self, jwe, error):
        with self.assertRaises(InvalidTokenException) as ite:
            decode_jwt(jwe, self.secret_store, purpose=KEY_PURPOSE_AUTHENTICATION, check_claims=self.CHECK_CLAIMS)
